...
export function CandidateTable({ candidates, onSelectCandidate, decided = false }: CandidateTableProps) {
  ...
  const rowMuted = decided ? "opacity-60" : "";
  ...
  return (
    <div className="card">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-left text-text-secondary">
              ...
              {decided ? (
                <th className="pb-3 pr-4">Karar</th>
              ) : (
                <th
                  className="pb-3 pr-4 cursor-pointer hover:text-text-primary"
          ...
          <tbody>
            {sorted.map((c) => (
              <tr
                ...
                className={`border-b border-border/50 hover:bg-surface-hover transition-colors cursor-pointer ${rowMuted} ${
              ...
              >
                ...
                {decided ? (
                  <td className="py-3 pr-4">
                    {c.decision_result ? (
                      <span className={`px-2 py-0.5 rounded-full font-semibold text-xs ${CHIP_STYLES[c.decision_result] || ""}`}>
                        {RECOMMENDATION_LABELS[c.decision_result] || c.decision_result}
                      </span>
                    ) : (
      ...
      <div className="md:hidden space-y-2">
        {sorted.map((c) => (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              {decided ? (
                c.decision_result ? (
                  <span className={`px-2 py-0.5 rounded-full font-semibold text-xs shrink-0 ${CHIP_STYLES[c.decision_result] || ""}`}>
                    {RECOMMENDATION_LABELS[c.decision_result] || c.decision_result}
                  </span>
                ) : (
                ...
                <span className={`px-2 py-0.5 rounded-full font-semibold text-xs shrink-0 ${CHIP_STYLES[c.evaluation.recommendation] || ""}`}>
                  {RECOMMENDATION_LABELS[c.evaluation.recommendation] || c.evaluation.recommendation}
                </span>
              ) : (
                <span className="text-text-muted text-xs shrink-0">—</span>
            ...
            <div className="flex items-center gap-3 mt-1.5 text-xs text-text-secondary">
              ...
              {!decided && (
                <span className="px-1.5 py-0.5 bg-surface-elevated rounded text-[10px]">{c.status}</span>
              )}
        ...
        {sorted.length === 0 && (
          <p className="py-8 text-center text-text-muted text-sm">No candidates found</p>
        )}
      </div>
    </div>
  );
